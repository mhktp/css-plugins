...
/*public Action event_PlayerSay(Event event, const char[] name, bool dontBreadcast) {
...
public void event_ClientConnect(Event event, const char[] name, bool dontBreadcast) {
	...
	char querry[256];
	FormatEx(querry, sizeof(querry),"SELECT is_muted FROM css.player_log WHERE networkid = '%s' AND is_muted = 1",clientAuthID);
	g_database.Query(QuerryCallback_IsMuted,querry,client);
	FormatEx(querry, sizeof(querry),"SELECT time_unban,networkid FROM css.player_log WHERE networkid = '%s' AND is_ban = 1",clientAuthID);
...
public Action cmd_Ban(int client, int args) {
	...
	char querry[256];
	FormatEx(querry, sizeof(querry),"UPDATE css.player_log SET is_ban = 1, time_unban = %d, number_ban = number_ban + 1, reason = '%s' WHERE networkid = '%s'",dbtime,reason,targetAuthID)
...
public Action cmd_Unban(int client, int args) {
	...
	char querry[256];
	FormatEx(querry, sizeof(querry),"UPDATE css.player_log SET is_ban = 0, time_unban = NULL WHERE networkid = '%s'",targetAuthID)
	g_database.Query(QuerryCallback_update,querry,client);
	return Plugin_Handled;
}
...
public void QuerryCallback_IsBan(Database hDatabase, DBResultSet results, const char[] sError, int client) {
	...
	while(results.FetchRow()) { // should be one row
		if(IsClientConnected(client)) {
			...
			// if not, just unban...
			char clientAuthID[32];
			results.FetchString(1,clientAuthID,sizeof(clientAuthID)); //networkid
			
			char querry[256];
			FormatEx(querry, sizeof(querry),"UPDATE css.player_log SET is_ban = 0, time_unban = NULL WHERE networkid = '%s'",clientAuthID)
			g_database.Query(QuerryCallback_update,querry,client);
		}	
	}
} 

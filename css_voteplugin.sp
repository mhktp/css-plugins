...
int gi_voteCooldown; // wait 1 minute for every player
...
public void event_ClientConnect(Event event, const char[] name, bool dontBreadcast) {
	...
	gi_voteCooldowns[client] = PLAYER_JOIN_COOLDOWN; // 120 seconds cooldown, this avoid players disconnect and reconnect to make another vote
}

public void event_ClientDisconnect(Event event, const char[] name, bool dontBreadcast) {
	int client = GetClientOfUserId(event.GetInt("userid"));		
	if(gb_voteInProgress) {
		gi_playerVotes[client] = 0;
...
/*public Action listener_Say(int client, const char[] command, int args) {
	...
	char querry[256];
	FormatEx(querry, sizeof(querry),"SELECT command FROM css.commands WHERE votecommand = 1 AND is_using = 1");
	...
	switch(action) {
		case MenuAction_Select: {
			...
			if (g_database == null) {
				...
				char querry[256];
				FormatEx(querry, sizeof(querry),"SELECT command,argsType,specifications,convar,admin_immunity FROM css.commands WHERE command = '%s'",item);
...
public void QuerryCallback_IntermediateVote(Database hDatabase, DBResultSet results, const char[] sError, int client) {
	...
	switch(argsType) 
	{
		...
		case 1 : { //args is a number
			...
			int convarInt =  GetConVarInt(cvCommand);
		...
		case 4: { // no args, we just need to execute the command.
			char querry[256];
			FormatEx(querry, sizeof(querry),"SELECT specifications,argsType,menutitle,timeapply,convar,'' FROM css.commands WHERE command = '%s'",gc_commandToVote[client]); 
	...
	switch(action) {
		case MenuAction_Select: {
			...
			if (g_database == null) {
				...
				char querry[256];
				FormatEx(querry, sizeof(querry),"SELECT specifications,argsType,menutitle,timeapply,convar,'%s' FROM css.commands WHERE command = '%s'",args,gc_commandToVote[clientVoter]); // HACK : since callbacks are allowed to one value, i'm parsing the args on the querry...
...
public void QuerryCallback_FinalVote(Database hDatabase, DBResultSet results, const char[] sError, int client) {
	...
	gi_voteCooldowns[client] = PLAYER_COOLDOWN; // 160 seconds is enough
	...
	for (int player = 1; player <= MaxClients; player++) 
	{
		...
		menu.Display(player,30); // amount of time in seconds
	}
...
public void VoteApply(){
	...
	float dividedVoters = float(gi_playerAllowToVote)/float(2);
	...
	FormatEx(request,sizeof(request),"SOCKETMANAGER|ADMIN|CSS Vote|Vote occured about '%s' by : %s (Networkid : %s) : Yes : %d ; No : %d (Numbers of voters : %d)",gc_commandToVote[gi_clientVoter],voterName,clientAuthID,numberOfTrue,numberOfFalse,gi_playerAllowToVote);
